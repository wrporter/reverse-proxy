...
scp ${SCP_PORT} $(pwd)/.ci/Caddyfile ${SSH_USER}@${SSH_HOST}:${BASE_DIRECTORY}${APP_NAME}/Caddyfile
...
ssh ${SSH_PORT_OPT} ${SSH_USER}@${SSH_HOST} "cd ${BASE_DIRECTORY}${APP_NAME} && docker-compose stop && docker-compose up --detach --build ${APP_NAME}"
